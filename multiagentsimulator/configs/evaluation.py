...
class EvaluationSettings:
    ...
    pandemic_regulations: Optional[List[ChosenRegulation]] = None
    default_sim_config: SimulationConfigs = small_town_config
    sim_opts: Optional[List[SimulationSettings]] = None
...
def evaluate_strategies(exp_name: str, eval_opts: EvaluationSettings) -> None:
    ...
    for i, strategy in enumerate(eval_opts.strategies):
        ...
        sim_opts = eval_opts.sim_opts[i] if eval_opts.sim_opts is not None else SimulationSettings()
...
def evaluate_spread_rates(exp_name: str, eval_opts: EvaluationSettings) -> None:
    ...
    for i, spread_rate in enumerate(eval_opts.spread_rates):
        ...
        sim_opts = SimulationSettings(infection_spread_rate_mean=spread_rate)
        experiment_main(sim_config=eval_opts.default_sim_config,
                        sim_opts=sim_opts,
                        data_saver=data_saver,
                        num_random_seeds=eval_opts.num_seeds,
...
def evaluate_testing_rates(exp_name: str, eval_opts: EvaluationSettings) -> None:
    assert eval_opts.pandemic_test_rate_scales is not None
    data_saver = H5DataSaver(exp_name, path=eval_opts.data_saver_path)
    def_sim_opts = SimulationSettings()
    ...
    for i, parameter_scale in enumerate(eval_opts.pandemic_test_rate_scales):
        ...
        sim_opts = SimulationSettings(random_testing_rate=def_sim_opts.random_testing_rate * parameter_scale,
                                      symp_testing_rate=def_sim_opts.symp_testing_rate * parameter_scale,
                                      retest_rate=def_sim_opts.retest_rate * parameter_scale)
        experiment_main(sim_config=eval_opts.default_sim_config,
                        sim_opts=sim_opts,
                        data_saver=data_saver,
                        pandemic_regulations=[ChosenRegulation(stay_home_if_sick=True, stage=0)],
                        stages_to_execute=0,
                        num_random_seeds=eval_opts.num_seeds,
                        max_episode_length=eval_opts.max_episode_length,
                        exp_id=i)
...
def evaluate_social_gatherings(exp_name: str, eval_opts: EvaluationSettings) -> None:
    assert eval_opts.avoid_gathering_sizes is not None
    data_saver = H5DataSaver(exp_name, path=eval_opts.data_saver_path)
    pandemic_regulations = [
        ChosenRegulation(risk_to_avoid_gathering_size={Risk.LOW: ags, Risk.HIGH: ags}, stage=stage)
...
def evaluate_location_contact_rates(exp_name: str, eval_opts: EvaluationSettings) -> None:
    assert eval_opts.social_distancing is not None
    data_saver = H5DataSaver(exp_name, path=eval_opts.data_saver_path)
    pandemic_regulations = [ChosenRegulation(social_distancing=sd, stage=stage)
...
def evaluate_population_sizes(exp_name: str, eval_opts: EvaluationSettings) -> None:
