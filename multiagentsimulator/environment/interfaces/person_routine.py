...
from .simulation_time import SimulationTimeInterval, SimulationTimeTuple, SimulationTime
...
class RoutineTrigger(metaclass=ABCMeta):
    ...
    @abstractmethod
    def trigger(self, sim_time: SimulationTime, person_state: Optional[PersonState] = None) -> bool:
        pass


class SimulationTimeRoutineTrigger(RoutineTrigger, SimulationTimeInterval):
...
class PersonRoutine:
    ...
    valid_time: SimulationTimeTuple = SimulationTimeTuple()
    """Specifies the time during which the routine is available to start."""

    start_trigger: RoutineTrigger = SimulationTimeRoutineTrigger(hour=1)
    ...
    reset_when_done_trigger: RoutineTrigger = SimulationTimeRoutineTrigger(day=1)
...
class PersonRoutineWithStatus:
    ...
    def _is_routine_due(self, sim_time: SimulationTime, person_state: Optional[PersonState]) -> bool:
    ...
    def sync(self, sim_time: SimulationTime, person_state: Optional[PersonState] = None) -> None:
